...
namespace BgTools.PlayerPrefsEditor
{
    public class PreferencesEditorWindow : EditorWindow
    {
        ...
        private string _prefHolder;
        private int _exportFileIndex;
...
#elif UNITY_EDITOR_OSX
            pathToPrefs =
 @"Library/Preferences/unity." + MakeValidFileName(PlayerSettings.companyName) + "." + MakeValidFileName(PlayerSettings.productName) + ".plist";
 ...
 @".config/unity3d/" + MakeValidFileName(PlayerSettings.companyName) + "/" + MakeValidFileName(PlayerSettings.productName) + "/prefs";
        ...
        private void InitReorderedList()
        {
            ...
            relSpliterPos =
                EditorPrefs.GetFloat("BGTools.PlayerPrefsEditor.RelativeSpliterPosition", 100 / position.width);

            userDefList.drawHeaderCallback = (Rect rect) => { EditorGUI.LabelField(rect, "User defined"); };
            ...
            userDefList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                ...
                EditorGUI.LabelField(new Rect(rect.x, rect.y, spliterPos - 1, EditorGUIUtility.singleLineHeight),
                    new GUIContent(prefKeyName, prefKeyName));
                GUI.enabled = false;
                EditorGUI.EnumPopup(new Rect(rect.x + spliterPos + 1, rect.y, 60, EditorGUIUtility.singleLineHeight),
                    (PreferenceEntry.PrefTypes)type.enumValueIndex);
                GUI.enabled = !showLoadingIndicatorOverlay;
                switch ((PreferenceEntry.PrefTypes)type.enumValueIndex)
                {
                    case PreferenceEntry.PrefTypes.Float:
                        EditorGUI.DelayedFloatField(
                            new Rect(rect.x + spliterPos + 62, rect.y, rect.width - spliterPos - 60,
            ...
            userDefList.onRemoveCallback = (ReorderableList l) =>
            {
                ...
                string prefKey = l.serializedProperty.GetArrayElementAtIndex(l.index).FindPropertyRelative("m_key")
                    .stringValue;
        ...
        void OnGUI()
        {
            ...
            try
            {
                ...
                GUIContent watcherContent = (entryAccessor.IsMonitoring())
                    ? new GUIContent(ImageManager.Watching, "Watching changes")
                    : new GUIContent(ImageManager.NotWatching, "Not watching changes");
                ...
                if (GUILayout.Button(new GUIContent("Export"), EditorStyles.toolbarButton))
                {
                    _exportFileIndex++;
                    _prefHolder = JsonConvert.SerializeObject(prefEntryHolder.userDefList);
                    var fileName = Path.Combine(Application.dataPath, $"playerprefs{_exportFileIndex}.txt");
                    try
                    {
                        ...
                        var data = new UTF8Encoding(true).GetBytes(_prefHolder);
                        fs.Write(data, 0, data.Length);
                        fs.Dispose();
                        fs.Close();

                        AssetDatabase.Refresh();
                        Debug.LogError($"Export Completed! Path:{fileName}");
                    }
                    catch (Exception Ex)
                    {
                        Debug.LogError($"Export Fail!");
                        Debug.LogError(Ex.ToString());
                    }
                }

                if (GUILayout.Button(new GUIContent("Import"), EditorStyles.toolbarButton))
                {
                    if (importObject == null)
                    {
                        Debug.LogError("Import file is missing!!");
                        GUIUtility.ExitGUI();
                    }

                    var asset = importObject as TextAsset;
                    var importHolder = JsonConvert.DeserializeObject<List<PreferenceEntry>>(asset.text);
                    PlayerPrefs.DeleteAll();

                    foreach (var entry in importHolder)
                    {
                ...
                importObject = EditorGUILayout.ObjectField(importObject, typeof(Object));
        ...
        private void CreatePrefEntries(string[] keySource, ref List<PreferenceEntry> listDest)
        {
            if (!string.IsNullOrEmpty(searchTxt) &&
                searchfield.SearchMode == MySearchField.SearchModePreferencesEditorWindow.Key)
            {
            ...
            if (!string.IsNullOrEmpty(searchTxt) &&
                searchfield.SearchMode == MySearchField.SearchModePreferencesEditorWindow.Value)
            {
                listDest = listDest
                    .Where((preferenceEntry) => preferenceEntry.ValueAsString().ToLower().Contains(searchTxt.ToLower()))
                    .ToList<PreferenceEntry>();
            }
...
public class MySearchField : SearchField
{
        ...
        GUIStyle emptyCancelButtonStyle)
    {
        ...
        if (Event.current.type == EventType.MouseDown && ContextMenuRect.Contains(Event.current.mousePosition))
        {
            ...
            {
                SearchMode = (SearchModePreferencesEditorWindow)Enum.Parse(typeof(SearchModePreferencesEditorWindow),
                    parameter.ToString());
                DropdownSelectionDelegate();
            }

            GenericMenu menu = new GenericMenu();
            foreach (SearchModePreferencesEditorWindow EnumIt in Enum.GetValues(
        ...
        GUI.Box(new Rect(rect.x, rect.y, 5, 5), EditorGUIUtility.IconContent("d_ProfilerTimelineDigDownArrow@2x"),
            ContexMenuOverlayStyle);

        if (!HasFocus() && String.IsNullOrEmpty(text))
        {
            GUI.enabled = false;
            GUI.Label(new Rect(rect.x + 14, rect.y, 40, rect.height),
                Enum.GetName(typeof(SearchModePreferencesEditorWindow), SearchMode));
            GUI.enabled = true;
        }
    ...
    public new string OnToolbarGUI(Rect rect, string text) => this.OnGUI(rect, text, EditorStyles.toolbarSearchField,
